		...
		.then(function (gasAmount) {
			payment_token.methods
				...
				.then(function (balance) {
					const token_decimals = get_currency_decimals_by_address(
						params.network,
						params.payment_token_address
					);
					let int_balance = BigInt(
						balance + "0".repeat(token_decimals)
					);
					...
					if (int_balance >= int_donation) {
						...
						payment_token.methods
							...
							.send({
								from: accounts[0],
								gasPrice: params["default_gas_price"],
								gas: 3 * gasAmount,
							})
							.on("error", function (error) {
								setErr(
									"Failed to get permission to send donation"
								);
							})
