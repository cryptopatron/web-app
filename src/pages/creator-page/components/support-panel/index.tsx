...
export default function SupportPanelComponent({ creatorDetails }) {
	...
	const [subscriptionPaymentDetails, setSubscriptionPaymentDetails] =
		useState({
			amount_per: 0,
			network: "",
			currency_name: "DAI",
	...
	function get_to_address(creator_details, network_value) {
		const addr: String = creator_details.metaMaskWalletPublicAddress;
		return addr ? addr : creator_details.generatedMaticWalletPublicAddress;
	}
	...
	return (
		<div
			className="flex flex-col mt-4 shadow-float-900 bg-white rounded-md text-center"
			style={{ width: "21rem" }}
		>
			<div className="font-semibold my-5">Support me!</div>
			{/* Stream and one time */}
			<div className="grid grid-cols-2 gap-y-0 gap-2 w-10/12 mx-auto">
				<button
					className={
						streamButtomActive ? " tab-active " : " tab-inactive "
					}
					onClick={() => {
				...
				<button
					className={
						streamButtomActive ? " tab-inactive " : " tab-active "
					}
					onClick={() => {
			...
			<div className="w-10/12 mx-auto">
				...
				{streamButtomActive ? (
					<div>
						...
						>
							<span>
								{"Stream " +
									String(
										subscriptionPaymentDetails.amount_per
									) +
									" " +
									String(
										subscriptionPaymentDetails.currency_name
									)}
							</span>
						</button>
					</div>
					...
					<div>
						...
						>
							<span>
								{"Send " +
									String(oneTimePaymentDetails.amount) +
									" " +
									oneTimePaymentDetails.currency_name}
							</span>
						</button>
					</div>
				)}
				...
				<div className="flex flex-col justify-center mt-3">
					<ListboxComponent
						content={network}
						setContent={setNetwork}
						ListboxContent={networks}
					/>
				</div>
				{/* Messages: */}
				<br></br>
