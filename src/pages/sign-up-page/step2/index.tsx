...
export default function Step2Component({
	...
	const [spaceAbsent, setSpaceAbsent] = useState(true);
	...
	const handleChange = (name) => {
		...
		if (name.includes(" ")) {
			setValid(false);
			setSpaceAbsent(false);
			setPageName(name);
			return;
		}
		setSpaceAbsent(true);

		// Make sure profile name is long enough
		if (name.length === 0) {
			setValid(false);
			setNeutral(true);
			setPageName(name);
		...
		if (isValid) {
			...
			if (isPresent) {
				if (wallet.wallet === "metamask") {
					...
					const status = await registerPage(
						pageName,
						{ metaMaskWalletPublicAddress: wallet.address },
						token
					);
					if (status) {
						moveToStep(3);
			...
			<div className="container mx-auto flex flex-col px-5 py-24 justify-center items-center">
				<div className="w-full md:w-2/3 flex flex-col mb-16 items-center text-center">
					<div className="text-center">
						<h2 className="title-font mb-6 font-semibold text-gray-900">
						...
						<p className=" text-gray-400 leading-relaxed mb-8">
							Remember your email from ten years ago?
						</p>
					</div>
					<div className="my-8 w-2/6 sm:w-40">
						<div
							className=" flex mx-auto justify-center top-16 w-32 h-32 rounded-full"
							style={{ background: `${profileBg[0]}` }}
						>
							<img
								className="w-2/5 transform translate-y-3"
								src={ImageDefaultProfile1}
					...
					<div className="w-68 sm:w-72 mt-3">
						<div className="flex w-full h-20 justify-center items-end mt-5">
							<div className="relative w-full">
								<div>
									{isNameShort ? (
										<label className={nameLengthCSS}>
											Profile name must have 4 or more
											characters
										</label>
									) : null}
									{isNamePresent ? null : (
										<label className={userPresentCSS}>
											Apologies! This name has already
											been taken.
										</label>
									)}
									{spaceAbsent ? null : (
										<label className={spaceInNameCSS}>
											Whoops! Spaces are not allowed.
										</label>
									)}
								</div>
								<div>
